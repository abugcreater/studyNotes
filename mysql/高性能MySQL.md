...
![img](https://1994385458-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M9s5bPe0j4A-Eb-j7Yp%2Fsync%2F9244b80fc408df72fcfd6ef636bab6d85a2847ef.png?generation=1606045678771959&alt=media)
...
![img](https://1994385458-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M9s5bPe0j4A-Eb-j7Yp%2Fsync%2Fb5ac98c770080e789a27675df88a981b00d26468.png?generation=1606302118931470&alt=media)

image-20201125163242573
...
![img](https://1994385458-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M9s5bPe0j4A-Eb-j7Yp%2Fsync%2Fbddd6d14551b3c1abc657f7cea4b4061d72813d4.png?generation=1606302116872034&alt=media)

image-20201125171652297
...
![img](https://1994385458-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M9s5bPe0j4A-Eb-j7Yp%2Fsync%2F27d5ca2220b339d317e377c7063e9df7ed8e84f8.png?generation=1606302119824249&alt=media)

image-20201125172420554

也不一定只使用一个分发主库，视情况而定。

还有一种更好的减轻负载的架构（金字塔形架构）：

![img](https://1994385458-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M9s5bPe0j4A-Eb-j7Yp%2Fsync%2F9f9b3470cbef001f8bd39f55ebb20e4b1a461496.png?generation=1606302120811242&alt=media)

image-20201125172646345

但是随着层级的增加，故障发生的几率与影响范围都会成倍递增。
...
先查看sync_binlog是否开启，如果开启了只需要指定备库从下一个二进制文件日志的开头读文件。如果没有开启这个选项，可能会造成部分日志的永久丢失，建议使用pt-table-checksum工具来检查主备一致性并进行修正。

- 

  备库意外关闭

通常没有办法，只能尝试使用pt-slave-restart工具去尝试修复

如果是InnoDB，会记录下错误日志，尝试从错误日志中获取日志应该处于的恢复点并指定POS到该恢复点并继续复制

如果主库二进制文件损坏，你只能跳过损坏的位置，使用`FLUSH LOGS`命令让主库开启一个新的文件并指定备库到这个新的二进制文件上来。并且需要通过`SET GLOBAL SQL_SLAVE_SKIP_COUNTER = 1`来忽略一个损坏的事件，如果存在多个损坏的事件，那么需要重复这个步骤多次。

不确定语句往往会给主备库之间造成数据差异，最常见的不确定语句如UPDATE配合LIMIT，由于不同存储引擎，数据排序可能出现差异，特别是在不同的存储引擎下该特性表现的更加明显，因此非常不建议数据表在主备库之间使用不同的存储引擎

备库必须显式声明server_id否则在执行START SLAVE时会报错

总结：

MySQL的复制功能显著增加了MySQL的功能和可用性。事实上这也正是MySQL这么快得以流行的关键原因之一。

### 第十一章、可扩展的MySQL

什么是可扩展性？

人们通常把可扩展性，高可用性以及性能等术语在一些非正式的场合用作同义词，但事实上他们完全是不同的

在第三章已经将性能定义为响应时间，简要的说，可扩展性表明了当需要增加资源以执行更多工作时系统能够获得划算的等价提升的能力。

我们往往不能直接看系统最大吞吐量，因为当系统到达最大吞吐量的时候，响应时间往往会变得不能接受，因此我们应该定义为在系统可以接受的响应时间里达到的吞吐量。

从较高层次看，可扩展性就是通过增加资源来提升容量的能力

一旦容量可以与服务器数量呈现线性关系，那么就认为扩展性非常优越，但是往往都无法达到只能尽量接近

对MySQL层面的扩展：如果只是将数据放在一个MySQL服务器当中，则迟早都会遇到性能瓶颈，当遇上性能瓶颈的时候传统的办法是买更强悍的机器，也就是我们通常说的垂直扩展或者是向上扩展，与之相反的是将任务分配到多个计算机上，这种扩展方法通常被称为水平扩展或者是

水平扩展划分为三个部分：复制、拆分和数据分片

最简单的水平扩展就是使用复制技术，将读查询的压力均匀的分配到备库上面去，但是也有缺点：例如重复缓存

数据库实现按功能拆分，将不同的表置于不同的服务器上，实现压力的平均

数据分片是扩展大型MySQL应用的方案中，是最通用也是最成功的方案。将数据拆分成一小块然后存储于不同的数据节点之上

> 如非必要，尽量不分片，因为分片会影响到性能调优或者数据库设计

数据分片的最大挑战是查找和获取数据，如何对这些数据进行分片操作，即选择出合适的分区键

使用分区键之后往往会导致查询缓慢等问题

如果对于特别复杂的数据类型，可以设置多个区分键来进行更好的区分。

跨分区的范围查询效率往往会非常低，建议使用Java或者C写一个辅助工具进行对几个数据块的并行查询。

分库分表就需要一个函数将数据分配到固定的分片当中，主要的两种方法有：固定分配和动态分配

1. 1.

   固定分配：

使用到的分区函数仅仅只是依赖分区键的值，哈希函数加上取模运算就是很好的分区函数，如目前有100个桶，可以使用如下的SQL语句来判定当前分区键应该存储于哪个哈希桶当中：

```
SELECT CRC32(111) % 100;
```

使用CRC32函数来计算哈希值然后取模选块

固定分配具有简单、开销低、良好适应缓存等特点
...
- 
  ...
  修改分配策略或者是一旦添加了新的哈希桶，数据修改起来会比较复杂

动态分配则是有点类似使用KV的形式将数据进行存储到数据表中。

设计数据分片时候，需要避免应用直接访问多数据源，而是需要编写一个抽象层，将数据隐藏于抽象层，抽象层主要完成以下几个任务：
...
不要让一台性能强悍的机器只运行一个实例，你可以让数据分片足够小，然后每台机器上运行多个分片（因为MySQL往往不会将性能发挥到极致）

随着云计算的扩展，动态扩展——根据负载或数据大小变化动态的在集群中增加/移动服务器——变得越来越有趣

负载均衡

思路：在一个服务器集群中尽可能平均负载均衡

通常做法是在服务器前端设置一个负载均衡器，然后负载均衡器将请求发送给最空闲的可用服务器

常见目的：可扩展性、高效性、可用性、透明性、一致性（如果应用是有状态的，如数据库事务，网络会话等，那么负载均衡会将相关的查询指向同一服务器，防止状态丢失）

![img](https://1994385458-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M9s5bPe0j4A-Eb-j7Yp%2Fsync%2F7f22c10aa82b7a0477e0d8492170ff4e6f9b17cc.png?generation=1606908641936682&alt=media)

image-20201202182836385

负载均衡适用于应用的任何一个层面上，可以在数据中心之间，多个MySQL 集群之间，同一个集群的MySQL之间使用负载均衡，适配高可用性
...
- 
  ...
  最快响应：能够最快处理请求的服务器接收下一个请求，响应时间存储于本地的缓存中

- 

  哈希：对连接的IP地址进行哈希运算来决定将请求打到哪一个服务器上
...
可以对读操作进行分区而不是随机分配来提高缓存命中效率（但是MySQL8.0取消了查询缓存，所以查询分区应该是无意义的）。

正确扩展MySQL架构并不是看起来那么容易并且美好，最好的策略是实现应用所指明需要的，并为可能的快速增长做好预先规划。

### 第十二章、高可用性

什么是高可用性？

高可用性通常以百分号表示，最终目标是100%但是永远也无法达到

总的来说，应用在超过一定的点以后追求更高的可用性是非常困难的

建立两个九或者是三个九的可用时间的目标可能并不困难

一个良好的应用架构通常可以降低可用性方面的需求，良好的架构也更容易做到高可用性

可以粗略使用宕机时间来衡量程序的可用性

引起MySQL宕机的原因：

表面原因是编写的SQL执行性能很差，其实不然：35%的事件属于运行环境问题，包括操作系统，磁盘网络等问题，35%属于性能问题，复制占20%，剩余的10%包括各种类型的数据丢失和损坏等
...
- 
  ...
  影响性能的第一因素就是糟糕的SQL语句，第二关键便是糟糕的Schema和索引设计
...
- 
  ...
  尽量保证在宕机时候能够尽快恢复，最常见的就是数据冗余，但是带来的负面影响便是资源投入比较高

避免单点失效问题，尽量快速解决宕机问题，或者增加系统冗余：增加余量空间和重复组件。

使用共享存储，将数据存储于类似于SAN的数据中心当中来，这样就可以集中精力关注一小部分组件获得高可用性。

对任何期望高可用的系统而言MyISAM都不是一个好的选择，因为只要一次崩溃都会导致表的损坏，表的修复工作也相当漫长。

MySQL同步复制（前面的复制都是异步复制）

同步复制保证了主库中的事务至少在一个备库上提交后才能认为其执行完成。

主要保证了主服务器崩溃时候至少有一个备库拥有实时的数据副本，大多数同步复制运行在主动-主动模式，可以非常轻易完成故障转移，这时候通过冗余获取到了高性能。

> MySQL不支持同步复制，得借由其他的工具来实现，可以考虑以下工具：
>
> - 
>
>   MySQL Cluster
...
购买来的服务资源来安装运行MySQL实例，可以根据需求随意配置MySQL和操作系统，但是无法知道底层的硬件信息

- 

  DBaaS（数据库即服务）

MySQL本身作为云端管理的资源，可以配置一些MySQL选项，但没有权限去配置或者查看底层操作系统或者虚拟服务器实例

讨论重点主要集中在第一类

本章关注的重点是：在云端运行MySQL还是在传统服务器上部署MySQL
...
- 
  ...
  云无需你自己管理基础设施如网络，磁盘等资源

- 

  云因为是即付即用的特新，可以将硬件部署的前期大量资本转换成持续的运营成本

- 

  云 具有强烈的时效性，随着时间的推移你可以花更少的钱来获取同样的配置
...
- 
  ...
  分配到的资源是不可预测的，这源于云的闲置资源共享特性

- 

  虚拟共享资源导致排查问题更加困难，提别是分析性能问题

云平台带来了便利，但是就性能，可用性和容量的透明性和控制力有所下降

对于处于初级阶段的企业，云具有很高的经济价值

通常用四种资源来衡量硬件系统：CPUI、内存、IO吞吐量和网络

通常，在云端，前三种资源的上限都无法和自定义服务器的资源上限相提并论，也就是如果MySQL 搭载到云端可能遇到前三种资源的性能瓶颈，但是网络资源基本不会遇到瓶颈

MySQL和InnoDB尤其依赖稳定的性能，提别是稳定的IO性能，因为IO操作会请求服务器内部的互斥锁，如果持有锁时间太长，很多线程就会阻塞起来，语句的执行时间更会成倍地增长。

对于部署于云端的服务器，如果CPU资源出现了瓶颈那么无能为力，但是如果是IO资源出现了瓶颈，则可以通过将数据存储进内存的方式来减少与磁盘的互动次数

### 第十四章、应用层优化

如果仅仅只是在MySQL的优化上花费太多时间，容易使得视野太过局限于MySQL本身，从而导致时间投入与性能产出占比过高。

> 本书既然是讲MySQL的，何故牵连上面的应用层呢？应用了MySQL的服务又不止一类，通用的优化往往无法提高对MySQL的利用率，个人看来这一章可以删去

性能监控工具New Relic

有些观点非常有趣：

应用是否执行了太多了查询？ORM宣称将用户从写SQL中解放出来的语句接口通常是罪魁祸首

对常见Web服务器如Apache的一些建议，如不要使用Apache存储静态资源，而是使用Nginx来替代静态资源的展示与读取等。

使用缓存可以获得数量级的性能提升，关键在于找到正确的粒度和缓存过期策略。

可以设置多级缓存，并不止发生在服务器上，可以存在于每一个环节。

缓存越接近客户端，缓存效率越高

应用层通常将缓存存储于同一台机器的内存中，或者是通过网络存储于另一台机器的内存中。

分布式缓存非常适合存储共享信息如：用户资料、评论、以及HTML片段
...
- 
  ...
  显式失效（如更新数据时候查询缓存主动失效）

一种有效方法是存储对象的时候顺带上版本号，这样可以保证缓存的时效性。但是如果版本更新太过频繁就会导致 缓存的命中率非常低。

MySQL并不擅长分布式数据处理，因为它缺乏并行执行查询能力

总结：

优化并不只是数据库的事儿，最高形式的优化既包含业务上的、也包含用户层的，全方位的优化才是最好的优化。

### 第十五章、备份与恢复

热备份、暖备份和冷备份：主要是表示一个备份的影响，如热备份就不需要任何的服务停机时间
...
最好的备份选择还是离线备份，关闭MySQL之后可以不用关心InnoDB缓存池中的脏页或者其他缓存，也不需要担心数据库备份过程中被修改

众多在线备份通常需要使用到FLUSH TABLES WITH READ LOCK操作，这会导致MySQL关闭并锁住所有的表，MyISAM还会将缓存刷新到磁盘上，但是InnoDB存储引擎不是这样的。虽然不像关闭服务器代价那么高昂，但是也是具有非常大的破坏性的。

避免使用FLUSH TABLES WITH READ LOCK最好的办法是只使用InnoDB表，因为一旦涉及到MyISAM的备份必然需要使用到上面的指令

两种主要的方式来备份MySQL数据库：逻辑备份（也叫导出，将数据导出成MySQL可以解析的格式，如SQL但不限制于SQL）和物理备份（直接复制原始数据文件）

互有利弊，建议间接性地使用两者进行备份工作

需要备份什么数据：

- 

  非显著数据，如：二进制文件和InnoDB事务日志等

- 

  代码：触发器和存储过程，通常都是随着数据库的备份而备份的，但是还原可能比较麻烦，因为还原单个数据库可能不会还原如存储过程，因为是存放于mysql数据库当中的

- 

  复制配置：如二进制文件、中继日志、日志索引文件和.info文件

- 

  服务器配置

> 不要因为会用高级备份数据而自负，解决方案越复杂，可能面临的风险也越大

服务器中的二进制文件是备份的最重要的信息之一，对基于时间点的恢复是必须的，并且占用空间通常比数据要小。所有可以轻易进行频繁的复制。可以使用mysqlbinlog指令完成复制

如果磁盘空间足够，建议保留二进制日志，不止对于数据恢复有帮助，对于分析服务器负载，审计都非常有用。

生成逻辑备份：

- 

  SQL导出，mysqldump默认的方式，但是mysqldump默认选项对生成一个巨大的备份却不是太合适的。
...
非常简单直接，如果是MyISAM存储引擎，往往只需要替换文件，并保证在替换文件的过程当中表不是处于打开的状态

InnoDB表则必须保证MySQL处于关闭的状态，并且需要事务日志文件与表空间文件匹配

以上是物理备份的还原过程，至于逻辑备份，则需要使用MySQL服务器本身来加载数据到表中

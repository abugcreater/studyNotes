...
从根节点configuration,开始遍历XNode,获取相关配置信息
...
配置存放在Properties，它继承自HashTable类, 所以碰到配置名称一致时会覆盖原有配置.
...
XMLConfigBuilder对于properties和environments的解析源码如下:
...
当DataSource使用占位符时如何解析,mybatis会使用PropertyParser这个类解析
...
默认实现的TypeHandler,每个都继承类抽象类BaseTypeHandler.如果需要自定义TypeHandler,也可以通过继承该抽象类实现.
...
我们在自定义TypeHandler的时候，可以在TypeHandler通过@MappedJdbcTypes指定jdbcType, 通过 @MappedTypes 指定javaType, 如果没有使用注解指定，那么我们就需要在配置文件中配置

### 2.6 元素5：objectFactory

MyBatis 每次创建结果对象的新实例时，它都会使用一个对象工厂（ObjectFactory）实例来完成。默认的对象工厂需要做的仅仅是实例化目标类，要么通过默认构造方法，要么在参数映射存在的时候通过参数构造方法来实例化.

如果需要自定义ObjectFactory,只需要继承DefaultObjectFactory,并重写器方法即可.
...
如果需要自定义plugin,则需要实现interceptor接口.
...
### 2.9 解析方法：objectFactoryElement、pluginElement、mapperElement
...
参考:[MyBatis详解 - 配置解析过程](https://pdai.tech/md/framework/orm-mybatis/mybatis-y-config-load.html)